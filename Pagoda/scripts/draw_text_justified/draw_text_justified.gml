...
for(var i = 0; i < array_height_2d(textJustified); i++)
{
	...
	if(string_contains(palavra, "|"))
	{
		...
		textJustified[i, INDEX_PALAVRAS] = palavra;
	}
	...
	if(string_contains(palavra, "#"))
	{
		...
		{
			draw_text(textJustified[j, INDEX_X] + (dif * index), textJustified[j, INDEX_Y], textJustified[j, INDEX_PALAVRAS]);
